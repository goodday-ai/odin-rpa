...
function summarizeAdminRateInventoryDryRunBatch({ mode = "ALL", startedAt, finishedAt, results = [] } = {}) {
  ...
  const safeResults = results.map((result) => ({
    ...
    uniquePlanCount: Number.isFinite(Number(result.uniquePlanCount)) ? Number(result.uniquePlanCount) : 0,
    uniquePlanItemCount: Number.isFinite(Number(result.uniquePlanItemCount)) ? Number(result.uniquePlanItemCount) : 0,
