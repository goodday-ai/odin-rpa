...
// 關鍵流程：深度遍歷 → 累積 room/date/plan context → 支援動態 numeric plan key → 建立 item → 計算 summary/diagnostics。
...
function itemFromContext(context, sourcePath) {
  ...
  if (context.inventory === null && context.maxInventory === null && context.explicitAvailable === null && context.isAllowBooking === null && context.isLock === null) return null;
  if (!context.salesUnitName && !context.roomTypeName && !context.planName && !context.channel) return null;

  const inventory = context.inventory === null ? (context.explicitAvailable === false || context.isLock === true || context.isAllowBooking === false ? 0 : Math.max(0, context.maxInventory || 0)) : context.inventory;
  const stockAvailable = inventory > 0 && context.isLock !== true;
  const available = context.isAllowBooking === false ? false : (context.explicitAvailable === false ? false : stockAvailable);
  const item = {
    date: context.date,
    ...
    inventory,
    maxInventory: context.maxInventory === null ? inventory : context.maxInventory,
    available,
...
function computeSummary(items, truncated = false) {
  ...
  for (const item of items) {
    ...
    if (!item.available) closedItemCount += 1;
  }
  return {
...
function normalizeCalendarsRateInventory(payload, options = {}) {
  ...
  function walk(value, inheritedContext, path, depth = 0, keyHint = "") {
    ...
    if (isDate && !isPlan && !hasChildPlan) addWarning("date context exists but no price plans", path);
