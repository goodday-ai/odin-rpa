...
const PLAN_ITEM_ID_KEYS = new Set(["item_id", "itemid", "plan_item_id", "planitemid"]);
const PLAN_ITEM_NAME_KEYS = new Set(["item_name", "itemname", "plan_item_name", "planitemname"]);
const RATE_PLAN_ID_KEYS = new Set(["rate_plan_id", "rateplanid"]);
const RATE_PLAN_NAME_KEYS = new Set(["rate_plan_name", "rateplanname"]);
const PACKAGE_ID_KEYS = new Set(["package_id", "packageid"]);
const PACKAGE_NAME_KEYS = new Set(["package_name", "packagename"]);
const POLICY_NAME_KEYS = new Set(["policy_name", "policyname"]);
const PLAN_ITEM_CONTAINER_KEYS = new Set(["plan_items", "planitems", "plan_item", "planitem", "rate_plan_items", "rateplanitems", "rate_plans", "rateplans", "packages", "package_items", "packageitems"]);
...
function isPlanItemContextObject(object, options = {}) {
  // 中文註解：子專案可能藏在 plan_items/rate_plans/packages，也可能只用 rate_plan_id/package_name 標示；集中判斷避免把上層 plan 誤降級。
  return Boolean(options.planItemContainer || options.numericPlanItemKey)
    || hasAnyKey(object, PLAN_ITEM_ID_KEYS)
    || hasAnyKey(object, PLAN_ITEM_NAME_KEYS)
    || hasAnyKey(object, RATE_PLAN_ID_KEYS)
    || hasAnyKey(object, RATE_PLAN_NAME_KEYS)
    || hasAnyKey(object, PACKAGE_ID_KEYS)
    || hasAnyKey(object, PACKAGE_NAME_KEYS)
    || hasAnyKey(object, POLICY_NAME_KEYS);
}
...
function collectLocalContext(object, options = {}) {
  ...
  const isPlanItem = isPlanItemContextObject(object, options);
  ...
  if (options.numericPlanItemKey) {
    local.sourcePlanItemKey = truncateText(options.numericPlanItemKey, 64);
    local.planItemId = truncateText(options.numericPlanItemKey, 64);
  }
  ...
  for (const [rawKey, rawValue] of Object.entries(object)) {
    ...
    // 中文註解：plan_items[] 內的 id/name 代表子專案，不可覆蓋父層 planId/planName，否則 dry-run 會看不到後台方案層級。
    if (isPlanItem) {
      if (PLAN_ITEM_ID_KEYS.has(token) || token === "id") local.planItemId = truncateText(rawValue, 64);
      if (PLAN_ITEM_NAME_KEYS.has(token) || NAME_KEYS.has(token)) local.planItemName = truncateText(rawValue);
      if (RATE_PLAN_ID_KEYS.has(token)) local.ratePlanId = truncateText(rawValue, 64);
      if (RATE_PLAN_NAME_KEYS.has(token)) {
        local.ratePlanName = truncateText(rawValue);
        if (!local.planItemName) local.planItemName = local.ratePlanName;
      }
      if (PACKAGE_ID_KEYS.has(token)) local.packageId = truncateText(rawValue, 64);
      if (PACKAGE_NAME_KEYS.has(token)) {
        local.packageName = truncateText(rawValue);
        if (!local.planItemName) local.planItemName = local.packageName;
      }
      if (POLICY_NAME_KEYS.has(token)) local.policyName = truncateText(rawValue);
    }

    if (!isPlanItem && isPlan && (PLAN_ID_KEYS.has(token) || token === "id")) local.planId = truncateText(rawValue, 64);
    ...
    if (!isPlanItem && isPlan && (PLAN_NAME_KEYS.has(token) || CHANNEL_NAME_KEYS.has(token) || NAME_KEYS.has(token))) {
...
function itemFromContext(context, sourcePath) {
  ...
  const item = {
    ...
    planItemName: context.planItemName || "",
    ratePlanId: context.ratePlanId || "",
    ratePlanName: context.ratePlanName || "",
    packageId: context.packageId || "",
    packageName: context.packageName || "",
    policyName: context.policyName || "",
    sourcePlanItemKey: context.sourcePlanItemKey || "",
...
function normalizeCalendarsRateInventory(payload, options = {}) {
  ...
  const items = [];
  let truncated = false;
  // 中文註解：dry-run 保持既有 500 筆防護；正式 snapshot sync 可透過 options.maxItems 放寬到 env 上限。
  const maxItems = Number.isFinite(Number(options.maxItems)) && Number(options.maxItems) > 0 ? Math.trunc(Number(options.maxItems)) : MAX_ITEMS;
  ...
  function walk(value, inheritedContext, path, depth = 0, keyHint = "") {
    if (depth > 24 || items.length >= maxItems) {
      truncated = true;
      return;
    }
    if (Array.isArray(value)) {
      value.forEach((child, index) => walk(child, inheritedContext, pathChild(path, index, true), depth + 1, keyHint));
      return;
    }
    if (!isPlainObject(value)) return;

    const numericPlanKey = isNumericKey(keyHint) && hasPlanPayload(value) ? keyHint : "";
    const numericPlanItemKey = numericPlanKey && inheritedContext.planId ? numericPlanKey : "";
    const numericPlanId = numericPlanKey && !numericPlanItemKey ? numericPlanKey : "";
    const planItemContainer = PLAN_ITEM_CONTAINER_KEYS.has(keyToken(keyHint));
    const local = collectLocalContext(value, { numericPlanId, numericPlanItemKey, planItemContainer });
    ...
    const isPlan = isPlanContextObject(value) || Boolean(numericPlanId) || Boolean(numericPlanItemKey) || planItemContainer;
    ...
    if ((numericPlanId || numericPlanItemKey) && !context.planName && !context.channel && !context.planItemName) addWarning("dynamic numeric key without name", path);
  ...
  const seen = new Set();
  for (const item of items) {
    const key = [item.date, item.salesUnitId, item.salesUnitName, item.roomTypeId, item.planId, item.planName, item.planItemId, item.planItemName, item.sourcePlanItemKey, item.channel, item.price, item.inventory, item.sourcePath].join("|");
    if (!seen.has(key)) {
      seen.add(key);
      deduped.push(item);
    }
