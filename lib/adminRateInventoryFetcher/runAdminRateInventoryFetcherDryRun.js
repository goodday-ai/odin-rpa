...
async function runAdminRateInventoryFetcherDryRun({ chromium, env = process.env } = {}) {
  ...
  try {
    ...
    request = buildCalendarsApiRequest({ origin: config.origin, hotelId: config.hotelId, start: config.start, end: config.end, lang: config.lang });
    if (validationErrors.length > 0) {
      const output = createControlledStopSnapshot(config, "invalid_config", { request: request.safe, warnings: validationErrors.map((reason) => ({ reason })) });
  ...
  try {
    ...
    const normalized = normalizeCalendarsRateInventory(fetched.json, { currency: "" });
