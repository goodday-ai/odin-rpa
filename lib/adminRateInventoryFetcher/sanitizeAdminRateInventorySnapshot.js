...
function createControlledStopSnapshot(config = {}, stoppedReason, extra = {}) {
  return sanitizeAdminRateInventorySnapshot({
    ...
    stoppedReason,
    summary: extra.summary || { itemCount: 0, dateCount: 0, salesUnitCount: 0, channelCount: 0, minPrice: null, maxPrice: null, availableItemCount: 0, zeroInventoryItemCount: 0 },
    items: [],
...
function auditSanitizedAdminRateInventoryOutDir(outDir = "out") {
  const files = fs.readdirSync(outDir)
    .filter((file) => /^admin_rate_inventory_fetcher_dryrun_.*\.json$/.test(file))
    .map((file) => path.join(outDir, file));
  if (files.length === 0) throw new Error("No admin rate inventory fetcher dry-run artifacts found");
