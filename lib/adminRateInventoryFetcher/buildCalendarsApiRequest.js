...
function buildCalendarsApiRequest({ origin, hotelId, start, end, lang = "zh_TW", maxDays = DEFAULT_MAX_RANGE_DAYS } = {}) {
  ...
  // 中文註解：不同呼叫端有不同安全上限；dry-run 不傳 maxDays 時維持 92 天，正式 snapshot sync 則由 config.maxDays 放寬到 120 天。
  if (!Number.isFinite(normalizedMaxDays) || normalizedMaxDays < 1) throw new Error("maxDays must be a positive number");
  const maxRangeDays = Math.trunc(normalizedMaxDays);
  if (rangeDays <= 0) throw new Error("start must be before end");
  if (rangeDays > maxRangeDays) throw new Error(`range must be <= ${maxRangeDays} days`);
