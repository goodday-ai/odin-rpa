...
function buildBaseOutput({ config, startedAt, durationMs, stoppedReason, stats, candidates, normalizedOffers, minPriceSummary }) {
...
async function runBookingPriceProbe({ chromium, env = process.env } = {}) {
  ...
  const output = buildBaseOutput({ config, startedAt, durationMs, stoppedReason, stats, candidates, normalizedOffers, minPriceSummary });
