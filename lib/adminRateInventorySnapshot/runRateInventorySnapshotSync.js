// 功能：執行 Admin Rate Inventory Snapshot Sync v1 的完整 goodday 同步流程。
// 責任：登入 Owlting 後台、抓 calendars API、normalize、建立 LINE 可讀 snapshot、成功才 publish latest、失敗只寫 out artifact。
...
async function runRateInventorySnapshotSync({ chromium, env = process.env } = {}) {
  const startedAt = Date.now();
  let config;
  try {
    config = loadEnabledTenantConfig(env);
  } catch (error) {
    const fallback = { tenant: env.RATE_INVENTORY_TENANT || "goodday", hotelId: "5720", start: "", end: "", outDir: env.RATE_INVENTORY_OUT_DIR || "out" };
    const artifact = controlledStopArtifact(fallback, "invalid_config", { warnings: [{ reason: String(error.message || error) }] });
    const outputPath = await writeArtifact(fallback, artifact).catch(() => "");
    return { ok: false, stoppedReason: "invalid_config", exitCode: 1, outputPath };
  }
  ...
  if (configErrors.length > 0) {
    const artifact = controlledStopArtifact(config, "invalid_config", { warnings: configErrors.map((reason) => ({ reason })) });
  ...
  const request = buildCalendarsApiRequest({ origin: config.origin, hotelId: config.hotelId, start: config.start, end: config.end, lang: config.lang });
  let browser;
  try {
  ...
  try {
    ...
    const artifact = { ...snapshot, published: publishResult.published, latestPath: publishResult.latestPath, dataBranch: publishResult.dataBranch, publishSkippedReason: publishResult.skippedReason };
    const outputPath = await writeArtifact(config, artifact);
    return { ok: true, stoppedReason: publishResult.skippedReason || "", exitCode: 0, outputPath, latestPath: publishResult.latestPath, published: publishResult.published };
  } catch (error) {
