...
// 關鍵流程：結構檢查 → 92 天 range 檢查 → allowlist 檢查 → summary 一致性 → sanitizer audit。
...
function validateRateInventorySnapshotForPublish(snapshot, config) {
  ...
  if (snapshot.tenant !== "goodday" || snapshot.tenant !== config.tenant) errors.push("tenant must be goodday");
  if (snapshot.hotelId !== "5720" || snapshot.hotelId !== config.hotelId) errors.push("hotelId must be 5720");
